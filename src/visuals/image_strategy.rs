...
/// - VirtualTexture: Single-draw-call multiscale rendering via a physical atlas + page table
...
impl ImageStrategy {
    ...
    pub fn prepare(
        ...
        match self {
            ...
            ImageStrategy::VirtualTexture(vt) => vt.prepare(queue, slice_plane, frame_number, camera_info),
        }
    }
    ...
    pub fn tiles(&self) -> &[Tile] {
        match self {
            ...
            ImageStrategy::VirtualTexture(_) => &[],
        }
    }
    ...
    pub fn dimensions(&self) -> (u32, u32, u32) {
        match self {
            ...
            ImageStrategy::VirtualTexture(vt) => vt.levels[0].volume_size,
        }
    }
    ...
    pub fn is_ready(&self) -> bool {
        match self {
            ...
            ImageStrategy::VirtualTexture(_) => true,
        }
    }

    /// Set LOD bias for automatic LOD selection (only for Tiled/VirtualTexture strategies)
    pub fn set_lod_bias(&mut self, bias: f32) {
        match self {
            ImageStrategy::Tiled(t) => t.lod_bias = bias,
            ImageStrategy::VirtualTexture(vt) => vt.lod_bias = bias,
            _ => {}
        }
    }
    ...
    pub fn set_debug_mode(&mut self, enabled: bool) {
        ...
        // No-op for Simple/VirtualTexture strategies
    }
    ...
    pub fn get_stats(&self) -> (usize, usize) {
        match self {
            ...
            ImageStrategy::VirtualTexture(vt) => (vt.slot_map.len(), vt.visible_tile_keys.len()),
        }
    }

    /// Provide chunk data for a requested tile (Tiled and VirtualTexture strategies)
    pub fn set_chunk_data(&mut self, lod_level: usize, z: u32, y: u32, x: u32, data: crate::visuals::tile::TileData) {
        ...
        match self {
            ImageStrategy::Tiled(t) => {
                t.requested_keys.remove(&key);
                t.pending_chunks.lock().unwrap().insert(key, data);
            }
            ImageStrategy::VirtualTexture(vt) => {
                vt.requested_keys.remove(&key);
                vt.pending_chunks.lock().unwrap().insert(key, data);
            }
            _ => {}
        }
    }
    ...
    pub fn get_ideal_lod(&self) -> usize {
        match self {
            ...
            ImageStrategy::VirtualTexture(vt) => vt.cached_ideal_lod,
        }
    }

    /// Get the pending chunks queue (Tiled and VirtualTexture strategies)
    pub fn pending_chunks(&self) -> Option<PendingChunks> {
        match self {
            ...
            ImageStrategy::VirtualTexture(vt) => Some(vt.pending_chunks.clone()),
        }
    }
}
...
pub struct VirtualTextureData {
    ...
    // Loader / pending-chunk infrastructure (mirrors TiledData)
...
impl VirtualTextureData {
    ...
    // ── Visibility / LOD selection (mirrors TiledData logic) ─────────────────
